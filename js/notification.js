(function ($) {
  ...
  var Notification = function (board, options) {
    ...
    this.hide = function () {
      ...
      htmlElement.animate({ opacity: 0.01 }, options.fadeOut, function () {
  ...
  $.createNotification = function (options) {
    ...
    var board = $(
      ".notification-board." + options.horizontal + "." + options.vertical
    );
    if (!board.length) {
      board = $(
    ...
    if (
      options.limit &&
      board.children(".notification:not(.hiding)").length >= options.limit
    ) {
      // limit reached
      if (options.queue) {
        queue.push(options);
